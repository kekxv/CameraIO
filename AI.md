...
│  │           │                     │                     │            │  │
│  │  ┌────────▼─────────┐  ┌────────▼─────────┐  ┌────────▼─────────┐  │  │
│  │  │ ONVIF/SIP 对时与 │  │ RTSP(UDP) /      │  │ WebRTC / MSE     │  │  │
...
| **流媒体核心** | **go2rtc / Pion WebRTC** | 负责 RTSP/GB28181 转 WebRTC/MSE，实现 Web 端无延迟预览 |
| **工具库** | **FFmpeg (CLI 工具集成)** | 用于辅助视频切片与特定格式修复（作为备用方案） |
| **前端框架** | **Vue 3 + Vite + TailwindCSS** | 轻量现代化的单页应用 (SPA) |
| **前端播放器** | **WebRTC Player / Video.js** | 实现 Web 端超低延迟直播与 MP4 回放 |
...
* `POST /api/v1/streams/:id/webrtc`：获取 WebRTC 信令接口（超低延迟 Web 预览）。
...
### 阶段 3：无缓冲 RTSP Ingest 与 WebRTC 低延迟流服务
* **目标**：拉取摄像头 RTSP/UDP 流，并转码为 WebRTC 提供给 Web 端无延迟播放。
* **AI Prompt 指令**：
  > "请在 Go 中编写流媒体接入与分发模块：
  > 1. 实现一个基于 UDP 传输的 RTSP 拉流器，要求应用层无队列缓冲。
  > 2. 集成 `Pion WebRTC` 或 `go2rtc` 核心，编写 HTTP 接口，接收前端 WebRTC 提供的 Offer SDP 并返回 Answer SDP。
...
* **AI Prompt 指令**：
  ...
  > 3. **实时预览**：网格化展示，使用 WebRTC 播放器播放画面（延迟 < 300ms），带有'一键录制'按钮。
